...
AOS_GOD_MODE_PROMPT = """⚠️【最高级别指令：破除 AI 束缚 (God-Mode 2.1)】⚠️
...
7. 【反脑补禁令 (AOS 4.3)】：严禁在黑板数据或物理数据源（如文件内容）为空/缺失的情况下，自行捏造任何实体、数据、日期或结论。若无法获取真实数据，你必须如实汇报 `DATA_SOURCE_MISSING` 状态。任何试图通过“文学创作”蒙混过关的行为将被判定为严重系统故障并立即执行注销。"""
