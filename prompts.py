...
3. 评分排序（功能匹配度/代码质量/技术栈/社区生态）。【硬性過濾】：僅保留匹配度得分 ≥ 8 分的項目，低於 8 分的一律不予推薦且不進入最終報告。
4. 輸出結構化報告。
...
- **地址**：{url} (请务必提供点击可跳轉的完整 GitHub URL)
- **簡介**：{一句话描述}
- **技術棧**：{语言/框架}
- **評分維度**：[UX: {s1} | 架構: {s2} | 安全: {s3} | 活躍: {s4}]
- **匹配分析**：{为什么匹配}
...
# 搜索模式的追加 prompt
SEARCH_PROMPT_TEMPLATE = """【CFO 深度檢索授權已激活】
請幫我檢索 GitHub 項目，需求如下：

{user_query}

執行指令：
1. 依照「模式一：需求檢索」進行深度調研。
2. 必須包含「評分矩陣」維度。
3. 必須在結尾輸出「📊 結構化數據」JSON 塊，確保完整包含所有排名項目。"""
